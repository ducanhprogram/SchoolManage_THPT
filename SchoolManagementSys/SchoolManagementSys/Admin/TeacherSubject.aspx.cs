...
namespace SchoolManagementSys.Admin
{
    public partial class TeacherSubject : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = fn.Fetch(@"Select ROW_NUMBER() over (Order by (Select 1)) as [Sr.No], ts.Id, ts.ClassId, c.ClassName, ts.SubjectId, s.SubjectName, ts.TeacherId, t.Name from 
                                    TeacherSubject ts inner join Class c on ts.ClassId = c.ClassId 
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt = fn.Fetch("Select *  from  TeacherSubject where ClassId = '" + classId + "' " +
                    "and SubjectId = N'" +subjectId + "' or TeacherId = '"+ teacherId +"' ");
                if (dt.Rows.Count == 0)
                {
                    string query = "Insert into TeacherSubject values('" + classId + "' ,N'" + subjectId + "', '"+teacherId+"')";
                    fn.Query(query);
                ...
                {
                    lblMsg.Text = "Giáo viên môn học đã nhập đã tồn tại ";
        ...
        protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                ...
                lblMsg.Text = "Giáo viên bộ môn xóa thành công!!!";
                lblMsg.CssClass = "alert alert-success";
                GridView1.EditIndex = -1;
                GetTeacherSubject();
